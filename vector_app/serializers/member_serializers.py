...
class OrgInviteCreateSerializer(serializers.Serializer):
    ...
    role = serializers.ChoiceField(
        choices=UserOrganization.ROLE_CHOICES, default=UserOrganization.ROLE_EDITOR
    )
    ...
    def validate_email(self, value):
        ...
        if user:
            existing = UserOrganization.objects.filter(user=user, organization=organization).exists()
            if existing:
...
class InviteAcceptSerializer(serializers.Serializer):
    ...
    def validate_token(self, value):
        ...
        if not invite:
            raise serializers.ValidationError("Invalid or expired invitation. Please request a new one.")

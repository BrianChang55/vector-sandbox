...
class SupabaseAdapter:
    ...
    def get_resource_schema(self, resource_id: str, ctx: AdapterContext) -> ResourceSchema:
        ...
        """
        try:
            schema_name, table_name = resource_id.split('.', 1) if '.' in resource_id else ('public', resource_id)
    ...
    def query(
        ...
        try:
            ...
            logger.error(f"HTTP error in query for {resource_id}: {e.response.status_code} - {e.response.text}")
            raise
        except Exception as e:
    ...
    def execute_action(
        ...
        try:
            ...
            if action_type == 'update_one':
                ...
                response = requests.patch(table_url, headers=headers, params=params, json=update_data, timeout=30)
                response.raise_for_status()
                
                data = response.json() if response.content else []
                result_data = data[0] if isinstance(data, list) and data else (data if not isinstance(data, list) else None)
            ...
            logger.error(f"HTTP error executing action {action_def.get('action_id')}: {e.response.status_code} - {e.response.text}")
            raise
        except Exception as e:

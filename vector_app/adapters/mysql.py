...
class MySQLAdapter:
    ...
    def get_resource_schema(self, resource_id: str, ctx: AdapterContext) -> ResourceSchema:
        ...
        try:
            schema_name, table_name = (
                resource_id.split(".", 1) if "." in resource_id else (ctx.database, resource_id)
            )
    ...
    def query(self, resource_id: str, query_spec: Dict[str, Any], user_ctx: UserContext) -> Dict[str, Any]:
        ...
        try:
            schema_name, table_name = (
                resource_id.split(".", 1) if "." in resource_id else (user_ctx.database, resource_id)
            )
    ...
    def execute_action(
        ...
        try:
            ...
            if action_type == "update_one":
                ...
                # Fetch updated row
                cursor.execute(
                    f"SELECT * FROM `{schema_name}`.`{table_name}` WHERE `{primary_key_field}` = %s",
                    [primary_key_value],
                )
                result = cursor.fetchone()

...
class AppDataService:
    ...
    @staticmethod
    def create_table(
        app: InternalApp, name: str, schema: dict, description: str = ""
        ...
        table = AppDataTable.objects.create(
            internal_app=app, name=name, slug=slug, description=description, schema_json=schema, row_count=0
        )
    ...
    def update_table_schema(
        table: AppDataTable, schema: dict, name: Optional[str] = None, description: Optional[str] = None
    ) -> tuple[Optional[AppDataTable], list[str]]:
        """
        ...
        if name is not None:
            ...
            counter = 1
            while (
                AppDataTable.objects.filter(internal_app=table.internal_app, slug=slug)
                .exclude(id=table.id)
                .exists()
            ):
                slug = f"{base_slug}-{counter}"
    ...
    def get_table(app: InternalApp, slug: str) -> Optional[AppDataTable]:
        ...
        """
        return AppDataTable.objects.filter(internal_app=app, slug=slug).first()
    ...
    @staticmethod
    def insert_row(table: AppDataTable, data: dict) -> tuple[Optional[AppDataRow], list[str]]:
        """
        ...
        with transaction.atomic():
            row = AppDataRow.objects.create(table=table, data=data_with_defaults)
            table.increment_row_count()

        return row, []

    @staticmethod
    def insert_rows_bulk(table: AppDataTable, rows_data: list[dict]) -> tuple[list[AppDataRow], list[str]]:
        """
        ...
        max_index = (
            AppDataRow.objects.filter(table=table).aggregate(max_index=Max("row_index"))["max_index"] or 0
        )
        ...
        with transaction.atomic():
            for i, data in enumerate(rows_data):
                ...
                # Create row
                max_index += 1
                row = AppDataRow(table=table, data=data_with_defaults, row_index=max_index)
    ...
    @staticmethod
    def update_row(row: AppDataRow, data: dict) -> tuple[Optional[AppDataRow], list[str]]:
        """
        ...
        # Check unique constraints
        unique_errors = AppDataService._check_unique_constraints(table, merged_data, exclude_row_id=row.id)
        if unique_errors:
            return None, unique_errors
    ...
    def delete_rows_bulk(table: AppDataTable, row_ids: list[uuid.UUID]) -> int:
        ...
        """
        with transaction.atomic():
            deleted_count, _ = AppDataRow.objects.filter(table=table, id__in=row_ids).delete()
            if deleted_count > 0:
    ...
    @staticmethod
    def query_rows(table: AppDataTable, query_spec: Optional[dict] = None) -> QueryResult:
        """
        ...
        for row in rows:
            ...
            if select_fields:
                row_data["data"] = {k: v for k, v in row.data.items() if k in select_fields}
            else:
    ...
    def _check_unique_constraints(
        ...
        for col in columns:
            if col.get("unique") and not col.get("primary_key"):
                ...
                exists_query = AppDataRow.objects.filter(table=table, **{f"data__{col_name}": value})
        ...
        for idx in indexes:
            if idx.get("unique"):
                ...
                # Check against database
                filter_kwargs = {f"data__{col}": val for col, val in idx_values.items()}
    ...
    def create_table_versioned(
        app: "InternalApp", version: "AppVersion", name: str, schema: dict, description: str = ""
    ) -> tuple[Optional["AppDataTable"], list[str]]:
        """
        ...
        AppDataTableSnapshot.create_snapshot(app_version=version, table=table, operation="create")
    ...
    def update_table_schema_versioned(
        ...
        table, errors = AppDataService.update_table_schema(table, schema, name, description)
        ...
        snapshot = AppDataTableSnapshot.create_snapshot(
            app_version=version, table=table, operation="update", previous_schema=previous_schema
        )
    ...
    def delete_table_versioned(
        table: "AppDataTable", version: "AppVersion", force: bool = False
        ...
        AppDataTableSnapshot.create_snapshot(app_version=version, table=table, operation="delete")
    ...
    @staticmethod
    def get_schema_at_version(table: "AppDataTable", version: "AppVersion") -> Optional[dict]:
        """
        ...
        snapshot = AppDataTableSnapshot.objects.filter(table=table, app_version=version).first()
    ...
    @staticmethod
    def get_tables_at_version(app: "InternalApp", version: "AppVersion") -> list[dict]:
        """
        ...
        snapshots = (
            AppDataTableSnapshot.objects.filter(app_version=version, table__internal_app=app)
    ...
    def rollback_tables_to_version(
        app: "InternalApp", target_version: "AppVersion"
    ) -> tuple[bool, list[str]]:
        """
        ...
        snapshots = AppDataTableSnapshot.objects.filter(
            app_version=target_version, table__internal_app=app
        ).select_related("table")

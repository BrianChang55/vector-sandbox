    ...
    def execute_steps(
        ...
        for wave_idx, wave in enumerate(waves):
            if len(wave) == 1:
                ...
                yield from self._execute_single_step(step, step_idx, step_executor, generated_files)
            else:
                # Multiple steps - execute in parallel
                logger.info(f"Executing wave {wave_idx} with {len(wave)} parallel steps")

                yield from self._execute_wave_parallel(wave, steps, step_executor, generated_files)

    ...
    while i < len(lines):
        ...
        if line.startswith("--- "):
            ...
            if current_path and (current_hunks or current_hunk_lines):
                ...
                diffs.append(
                    FileDiff(path=current_path, hunks=current_hunks, lines_added=added, lines_removed=removed)
                )
...
def _apply_with_subprocess_patch(original_content: str, diff_text: str) -> str:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = subprocess.run(
            ["patch", "--fuzz=3", "-o", out_file, orig_file, "-i", patch_file], capture_output=True, text=True
        )

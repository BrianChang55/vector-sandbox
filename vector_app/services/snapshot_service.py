...
class SnapshotService:
    ...
    def get_snapshot_diff(
        ...
        files_modified = [
            p for p in to_files 
            if p in from_files and to_files[p] != from_files[p]
        ]
    ...
    def execute_rollback(
        ...
        with transaction.atomic():
            ...
            schema_changes = None
            if include_schema:
                schema_changes = SnapshotService._revert_schemas(
                    app, target_version, rollback_version
                )

...
class AppSpecValidationService:
    ...
    def validate_app_spec(app: InternalApp, spec_json: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        for i, page in enumerate(pages):
            page_errors = AppSpecValidationService._validate_page(page, resource_ids, enabled_resources, i)
    ...
    def _validate_page(
        ...
        resource_entry = next((r for r in enabled_resources if r.resource_id == primary_resource), None)
    ...
    def _validate_table_view(
        table: Dict[str, Any], resource_entry: ResourceRegistryEntry, prefix: str
    ) -> List[str]:
        ...
        for i, action in enumerate(bulk_actions):
            ...
            if action_id and action_id not in allowed_actions:
                errors.append(
                    f"{prefix}.view.table.bulkActions[{i}]: action '{action_id}' is not allowlisted"
                )
    ...
    def _validate_detail_drawer(
        drawer: Dict[str, Any], resource_entry: ResourceRegistryEntry, prefix: str
    ) -> List[str]:
        ...
        for i, action in enumerate(actions):
            ...
            if action_id and action_id not in allowed_actions:
                errors.append(
                    f"{prefix}.view.detailDrawer.actions[{i}]: action '{action_id}' is not allowlisted"
                )

...
class VersionService:
    ...
    def get_latest_stable_version(app: InternalApp) -> Optional[AppVersion]:
        ...
        return (
            AppVersion.objects.filter(
                internal_app=app, generation_status=AppVersion.GEN_STATUS_COMPLETE, is_active=True
            )
    ...
    def get_latest_version(app: InternalApp) -> Optional[AppVersion]:
        ...
        """
        return AppVersion.objects.filter(internal_app=app).order_by("-version_number").first()
    ...
    def cancel_generating_version(version_id: str, user=None) -> dict:
        ...
        try:
            ...
            return {"success": False, "error": "Version not found", "action": None}

        # If already complete, nothing to do
        if version.generation_status == AppVersion.GEN_STATUS_COMPLETE:
            return {"success": True, "action": "already_complete", "version_id": str(version.id)}

        # If generating or pending, cancel it
        if version.generation_status in [AppVersion.GEN_STATUS_GENERATING, AppVersion.GEN_STATUS_PENDING]:
            with transaction.atomic():
                ...
                if file_count == 0:
                    ...
                    return {"success": True, "action": "deleted", "version_number": version_number}
                else:
        ...
        if version.generation_status == AppVersion.GEN_STATUS_ERROR:
            return {"success": True, "action": "already_error", "version_id": str(version.id)}
    ...
    def cleanup_stale_generating_versions(app: InternalApp, max_age_minutes: int = 30) -> int:
        ...
        stale_versions = AppVersion.objects.filter(
            internal_app=app, generation_status=AppVersion.GEN_STATUS_GENERATING, created_at__lt=cutoff
        )

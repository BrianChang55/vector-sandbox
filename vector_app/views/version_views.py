...
class AppVersionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def ai_edit(self, request, internal_app_pk=None):
        ...
        try:
            ...
            return Response(
                {"error": f"AI generation failed: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
        ...
        if not is_valid:
            return Response(
                {"error": "AppSpec validation failed", "errors": errors}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def code_edit(self, request, internal_app_pk=None):
        ...
        if file_path not in CodegenService.ALLOWLISTED_PATHS:
            return Response(
                {"error": f"File path not allowlisted: {file_path}"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def rollback(self, request, pk=None):
                ...
                if include_schema:
                    try:
                        ...
                        schema_compatibility = SchemaMigrationService.check_table_compatibility(
                            current_snapshot.tables_json, target_snapshot.tables_json
                        )
        ...
        try:
            ...
            return Response(AppVersionSerializer(rollback_version).data, status=status.HTTP_201_CREATED)
        except Exception as e:
            ...
            return Response(
                {"error": f"Rollback failed: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )

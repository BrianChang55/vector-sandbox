...
class RuntimeDataProxyView(APIView):
    ...
    def post(self, request):
        ...
        if not table_slug and operation not in ("listTables",):
            return Response(
                {"error": "tableSlug is required for this operation"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response({"error": f'Table "{table_slug}" not found'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def _get_schema(self, table: AppDataTable, version_id: str = None) -> dict:
        ...
        if version_id:
            try:
                snapshot = AppDataTableSnapshot.objects.get(table=table, app_version_id=version_id)
    ...
    def _handle_get_schema(
        self, app: InternalApp, table: AppDataTable, schema: dict, params: dict
    ) -> Response:
        """Get table schema."""
        return Response(
            {
    ...
    def _handle_query(self, app: InternalApp, table: AppDataTable, schema: dict, params: dict) -> Response:
    ...
    def _handle_insert(self, app: InternalApp, table: AppDataTable, schema: dict, params: dict) -> Response:
        """Insert a new row."""
        data = params.get("data", {})

        if not data:
            return Response(
                {"error": "data is required for insert operation"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if errors:
            return Response(
                {"error": "Validation failed", "errors": errors}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def _handle_update(self, app: InternalApp, table: AppDataTable, schema: dict, params: dict) -> Response:
        ...
        if not row_id:
            return Response(
                {"error": "rowId is required for update operation"}, status=status.HTTP_400_BAD_REQUEST
            )

        if not data:
            return Response(
                {"error": "data is required for update operation"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if errors:
            return Response({"error": "Update failed", "errors": errors}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def _handle_delete(self, app: InternalApp, table: AppDataTable, schema: dict, params: dict) -> Response:
        """Delete a row."""
        row_id = params.get("rowId")

        if not row_id:
            return Response(
                {"error": "rowId is required for delete operation"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def _handle_bulk_insert(
        ...
        if len(rows_data) > 1000:
            return Response(
                {"error": "Maximum 1000 rows per bulk insert"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def _handle_bulk_delete(
        ...
        if len(row_ids) > 1000:
            return Response(
                {"error": "Maximum 1000 rows per bulk delete"}, status=status.HTTP_400_BAD_REQUEST
            )

...
def get_user_org_membership(user, organization):
    ...
    return UserOrganization.objects.filter(user=user, organization=organization).first()
    ...
    def patch(self, request, org_id, member_id):
        ...
        try:
            ...
            if not require_org_admin(request.user, organization):
                return Response(
                    {"error": "Only admins can update member roles"}, status=status.HTTP_403_FORBIDDEN
                )

            # Get the member to update
            member = get_object_or_404(UserOrganization, id=member_id, organization=organization)

            serializer = OrgMemberUpdateSerializer(
                data=request.data, context={"request": request, "member": member}
            )
        ...
        try:
            ...
            if member.is_admin():
                admin_count = UserOrganization.objects.filter(
                    organization=organization, role=UserOrganization.ROLE_ADMIN
                ).count()
                if admin_count <= 1:
                    return Response(
    ...
    def post(self, request, org_id):
        ...
        try:
            ...
            serializer = OrgInviteCreateSerializer(
                data=request.data, context={"request": request, "organization": organization}
            )
...
class OrgInviteDetailView(APIView):
    ...
    def delete(self, request, org_id, invite_id):
        ...
        try:
            ...
            if not require_org_admin(request.user, organization):
                return Response(
                    {"error": "Only admins can cancel invitations"}, status=status.HTTP_403_FORBIDDEN
                )

            invite = get_object_or_404(
                OrganizationInvite, id=invite_id, organization=organization, is_accepted=False
            )
...
class OrgInviteResendView(APIView):
    ...
    def post(self, request, org_id, invite_id):
        ...
        try:
            ...
            if not require_org_admin(request.user, organization):
                return Response(
                    {"error": "Only admins can resend invitations"}, status=status.HTTP_403_FORBIDDEN
                )
            ...
            logger.info(
                f"Invitation resent to {new_invite.email} for {organization.name} by {request.user.email}"
            )

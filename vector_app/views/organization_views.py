...
class OrganizationViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        user_org = UserOrganization.objects.filter(
            user=request.user, organization=org, role=UserOrganization.ROLE_ADMIN
        ).first()

        if not user_org:
            return Response(
                {"error": "Only admins can update organization settings"}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def logo(self, request, pk=None):
        ...
        if not user_org:
            return Response(
                {"error": "Only admins can upload organization logo"}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def delete_logo(self, request, pk=None):
        ...
        if not user_org:
            return Response(
                {"error": "Only admins can delete organization logo"}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if not user_org:
            return Response(
                {"error": "Only admins can delete the organization"}, status=status.HTTP_403_FORBIDDEN
            )

...
class RuntimeQueryView(APIView):
    ...
    def post(self, request):
        ...
        if not all([app_id, resource_id]):
            return Response(
                {"error": "appId and resourceId are required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if version and version.scope_snapshot_json:
            ...
            registry_entry = ResourceRegistryEntry.objects.filter(
                resource_id=resource_id, backend_connection=app.backend_connection, enabled=True
            ).first()
        ...
        if app.backend_connection.adapter_type == BackendConnection.ADAPTER_SUPABASE:
            user_auth = ProjectUserBackendAuth.objects.filter(
                user=request.user, backend_connection=app.backend_connection
            ).first()
...
class RuntimeActionView(APIView):
    ...
    def post(self, request):
        ...
        if not all([app_id, action_id]):
            return Response({"error": "appId and actionId are required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not app.allow_actions_in_preview:
            return Response(
                {"error": "Actions are not allowed in preview mode"}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        if not action_def:
            return Response(
                {"error": f"Action {action_id} is not allowlisted"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if input_schema:
            try:
                ...
                return Response(
                    {"error": f"Invalid action arguments: {e.message}"}, status=status.HTTP_400_BAD_REQUEST
                )

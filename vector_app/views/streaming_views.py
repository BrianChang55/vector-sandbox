...
class ChatMessagesView(APIView):
    ...
    def get(self, request, session_id):
        ...
        try:
            session = ChatSession.objects.select_related("internal_app__organization").get(pk=session_id)
...
class StreamingGenerateView(View):
    ...
    def get(self, request, app_id):
        ...
        try:
            app = InternalApp.objects.select_related("organization", "backend_connection").get(pk=app_id)
        except InternalApp.DoesNotExist:
        ...
        try:
            ...
            if not membership.is_editor_or_above():
                return JsonResponse({"error": "You must be an editor or admin to generate code"}, status=403)
...
class AgenticGenerateView(View):
    ...
    def get(self, request, app_id):
        ...
        response = StreamingHttpResponse(self._stream_from_job(job), content_type="text/event-stream")
        response["Cache-Control"] = "no-cache"
        response["X-Accel-Buffering"] = "no"
        return response
...
class JobStreamView(View):
    ...
    def get(self, request, job_id):
        ...
        try:
            job = CodeGenerationJob.objects.select_related("internal_app__organization").get(pk=job_id)
        ...
        response = StreamingHttpResponse(
            self._stream_events(job, last_index), content_type="text/event-stream"
        )
        response["Cache-Control"] = "no-cache"
        response["X-Accel-Buffering"] = "no"
        return response
...
class JobStatusView(APIView):
    ...
    def get(self, request, job_id):
        ...
        # Verify access
        if not request.user.user_organizations.filter(organization=job.internal_app.organization).exists():
            return Response({"error": "Access denied"}, status=http_status.HTTP_403_FORBIDDEN)
...
class LatestJobView(APIView):
    ...
    def get(self, request, app_id):
        ...
        active_job = (
            CodeGenerationJob.objects.filter(
                internal_app=app,
                status__in=[
                    CodeGenerationJob.STATUS_QUEUED,
                    CodeGenerationJob.STATUS_PROCESSING,
        ...
        latest_job = CodeGenerationJob.objects.filter(internal_app=app).order_by("-created_at").first()
...
class GenerationStateView(APIView):
    ...
    def get(self, request, version_id):
        ...
        try:
            version = (
                AppVersion.objects.select_related("internal_app__organization")
            ...
            return Response({"error": "Version not found"}, status=http_status.HTTP_404_NOT_FOUND)
...
class LatestGenerationView(APIView):
    ...
    def get(self, request, app_id):
        ...
        try:
            ...
            version = (
                AppVersion.objects.filter(internal_app=app)
                .prefetch_related("files")
                .order_by("-created_at")
                .first()
            )
...
class ApplyGeneratedCodeView(APIView):
    ...
    def post(self, request, message_id):
        ...
        try:
            message = ChatMessage.objects.select_related("session__internal_app__organization").get(
                pk=message_id
            )
            ...
            if not spec_json:
                return Response(
                    {"error": "No generated spec in this message"}, status=http_status.HTTP_400_BAD_REQUEST
                )
...
class CancelGenerationView(APIView):
    ...
    def post(self, request, version_id):
        ...
        try:
            version = AppVersion.objects.select_related("internal_app__organization").get(pk=version_id)
                ...
                return Response(
                    {
                        ...
                        "latest_stable_version_id": str(latest_stable.id) if latest_stable else None,
                        "latest_stable_version_number": (
                            latest_stable.version_number if latest_stable else None
                        ),
                    }
                )
            else:
                return Response(
                    {"error": result.get("error", "Cancel failed")}, status=http_status.HTTP_400_BAD_REQUEST
                )
...
class FixErrorsView(View):
    ...
    def get(self, request, version_id):
        ...
        if not errors_b64:
            return JsonResponse({"error": "No errors provided"}, status=400)
        ...
        try:
            membership = UserOrganization.objects.get(
                user=user, organization=version.internal_app.organization
            )
            if not membership.is_editor_or_above():
                return JsonResponse({"error": "You must be an editor or admin to fix errors"}, status=403)
        except UserOrganization.DoesNotExist:
            return JsonResponse({"error": "Access denied"}, status=403)
            ...
            if fixed_files_by_path:
                for path, file_data in fixed_files_by_path.items():
                    VersionFile.objects.update_or_create(
                        app_version=version, path=path, defaults={"content": file_data["content"]}
                    )

...
class RuntimeConnectorProxyView(APIView):
    ...
    def post(self, request):
        ...
        if not connector_id:
            return Response({"error": "connectorId is required"}, status=status.HTTP_400_BAD_REQUEST)

        if not tool_id:
            return Response({"error": "toolId is required"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def _handle_meta_operation(
        self, provider: MergeIntegrationProvider, tool_id: str, params: dict
    ) -> Response:
        ...
        if tool_id == "list":
            ...
            if not connector_id:
                return Response(
                    {"error": "connectorId is required in params"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            try:
                ...
                return Response(
                    {"error": f'Connector "{connector_id}" not found'}, status=status.HTTP_404_NOT_FOUND
                )
            ...
            if not provider.merge_registered_user_id:
                return Response(
                    {"error": "Organization is not registered with Merge"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            if not tool_name:
                return Response(
                    {"error": "tool_name is required in params"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            try:
                ...
                if result.success:
                    ...
                    if error_in_content:
                        logger.warning(
                            f"MCP tool '{tool_name}' returned error in content: {error_in_content}"
                        )
                        return Response(
                            {
                                "success": False,
            ...
            return Response(
                {"error": f"Unknown meta operation: {tool_id}"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def _execute_tool(
        ...
        try:
            ...
            ConnectorExecutionLog.objects.create(
                ...
                status=(
                    ConnectorExecutionLog.STATUS_SUCCESS
                    if result.success
                    else ConnectorExecutionLog.STATUS_ERROR
                ),

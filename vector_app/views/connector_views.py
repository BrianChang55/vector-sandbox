...
def require_admin_for_integrations(request, organization):
    ...
    """
    membership = UserOrganization.objects.filter(user=request.user, organization=organization).first()
...
def sync_connectors(request, pk):
    ...
    if not require_admin_for_integrations(request, provider.organization):
        return Response({"error": "Only admins can manage integrations"}, status=status.HTTP_403_FORBIDDEN)
...
def list_connectors(request, pk):
        ...
        try:
            ...
            for connector_data in connectors_data:
                ...
                for tool_data in tools:
                    ...
                    if action_type not in tools_by_action:
                        tools_by_action[action_type] = []
                    tools_by_action[action_type].append(tool_id)
                    total_tools += 1
    ...
    org_connections = {}
    for link in OrganizationConnectorLink.objects.filter(provider=provider).select_related(
...
def handle_link_callback(request, pk):
    ...
    connector_id = request.data.get("connector_id")
    if not connector_id:
        return Response({"error": "connector_id is required"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        return Response({"error": f'Connector "{connector_id}" not found'}, status=status.HTTP_404_NOT_FOUND)
...
def get_tool_pack_info(request, pk):
    ...
    if not is_merge_configured():
        return Response(
            {"error": "Integrations are not configured"}, status=status.HTTP_503_SERVICE_UNAVAILABLE
        )

...
class RuntimeQueryView(APIView):
    ...
    def post(self, request):
        ...
        if not all([app_id, version_id, resource_id]):
            return Response(
                {"error": "appId, versionId, and resourceId are required"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class RuntimeActionView(APIView):
    ...
    def post(self, request):
        ...
        if not all([app_id, version_id, action_id]):
            return Response(
                {"error": "appId, versionId, and actionId are required"}, status=status.HTTP_400_BAD_REQUEST
            )

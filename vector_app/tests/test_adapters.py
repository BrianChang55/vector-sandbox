...
from vector_app.adapters.base import (
    AdapterContext,
    UserContext,
    Resource,
    ResourceField,
...
class TestAdapterContext(TestCase):
    ...
    def test_supabase_context(self):
        ...
        ctx = AdapterContext(
            backend_url="https://example.supabase.co", service_role_key="secret-key", anon_key="anon-key"
        )
    ...
    def test_mysql_context(self):
        ...
        ctx = AdapterContext(
            host="localhost", port=3306, database="testdb", username="root", password="secret"
        )
        self.assertEqual(ctx.host, "localhost")
...
class TestUserContext(TestCase):
    ...
    def test_supabase_user_context(self):
        ...
        ctx = UserContext(
            backend_url="https://example.supabase.co", user_jwt="jwt-token", anon_key="anon-key"
        )
        self.assertEqual(ctx.backend_url, "https://example.supabase.co")
    ...
    def test_postgresql_user_context(self):
        ...
        ctx = UserContext(
            host="localhost", port=5432, database="testdb", username="postgres", password="secret"
        )
        self.assertEqual(ctx.host, "localhost")
        self.assertEqual(ctx.port, 5432)
...
class TestResourceDataclasses(TestCase):
    ...
    def test_resource_field(self):
        ...
        field = ResourceField(name="email", type="string", nullable=False, primary_key=False)
    ...
    def test_resource_field_with_primary_key(self):
        ...
        field = ResourceField(name="id", type="integer", nullable=False, primary_key=True)
        self.assertTrue(field.primary_key)

    def test_relationship(self):
        """Test Relationship dataclass."""
        rel = Relationship(
            from_field="user_id", to_resource_id="public.users", to_field="id", type="many_to_one"
        )
    ...
    def test_resource(self):
        ...
        resource = Resource(id="public.users", name="users", kind="table", namespace="public")
    ...
    def test_resource_schema(self):
        ...
        relationships = [Relationship("org_id", "public.orgs", "id", "many_to_one")]
        schema = ResourceSchema(fields=fields, relationships=relationships, capabilities=["read", "update"])
...
class TestPostgreSQLAdapter(TestCase):
    ...
    def test_query(self, mock_connect):
        ...
        query_spec = {"select": ["id", "email", "name"], "limit": 10}
...
class TestQuerySpecParsing(TestCase):
    ...
    def test_query_spec_with_pagination(self, mock_connect):
        ...
        query_spec = {"select": ["id"], "limit": 10, "offset": 20}

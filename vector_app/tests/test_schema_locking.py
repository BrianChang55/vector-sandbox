...
from vector_app.models import User, Organization, UserOrganization, InternalApp, AppVersion, AppDataTable
...
class TestSchemaLocking(TestCase):
    ...
    def setUp(self):
        ...
        UserOrganization.objects.create(
            user=self.user, organization=self.org, role=UserOrganization.ROLE_ADMIN
        )
        ...
        self.version = AppVersion.objects.create(
            internal_app=self.app, version_number=1, spec_json={}, created_by=self.user
        )
    ...
    def test_schema_locking_prevents_table_updates_during_generation(self):
        ...
        events = list(self.handler._apply_table_definitions(table_defs, self.app, self.version))
    ...
    def test_new_table_creation_still_works(self):
        ...
        table = AppDataTable.objects.filter(internal_app=self.app, slug="new-table").first()
...
class TestTableCodeSeparation(TestCase):
    ...
    @patch.object(GenerateHandler, "create_streaming_validator")
    @patch.object(GenerateHandler, "stream_llm_response")
    def test_early_return_when_tables_created(self, mock_stream, mock_validator):
        ...
        mock_validator_instance.check_chunk.return_value = []
        mock_validator_instance.final_check.return_value = []
        mock_validator.return_value = mock_validator_instance

        step = PlanStep(id="step-1", type="data", title="Define data tables", description="Create task table")

        # Execute step
        events = list(
            self.handler._execute_step(
                step=step,
                step_index=0,
        ...
        table = AppDataTable.objects.filter(internal_app=self.app, slug="tasks").first()
        self.assertIsNotNone(table)
        self.assertEqual(table.slug, "tasks")
        self.assertEqual(table.name, "Tasks")

        # Should have emitted some events (thinking, table_created)
        ...
        self.assertEqual(
            len(file_events), 0, "Should not generate files when tables are created (early return)"
        )

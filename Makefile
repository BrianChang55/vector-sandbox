.PHONY: help format format-all format-check format-check-all lint lint-all static static-all check check-all check-diff install-static clean
...
format:
	...
	if [ -z "$$CHANGED_FILES" ]; then \
		...
		echo "Running isort..."; \
		isort --line-length 110 --profile black $$CHANGED_FILES; \
		echo "Running black..."; \
		black --line-length 110 $$CHANGED_FILES; \
...
format-check:
	...
	if [ -z "$$CHANGED_FILES" ]; then \
		...
		echo "$$CHANGED_FILES" | tr ' ' '\n' | sed 's/^/  - /'; \
		isort --line-length 110 --profile black --check-only --diff $$CHANGED_FILES; \
		black --line-length 110 --check --diff $$CHANGED_FILES; \
...
lint:
	...
	if [ -z "$$CHANGED_FILES" ]; then \
		...
		echo "$$CHANGED_FILES" | tr ' ' '\n' | sed 's/^/  - /'; \
		prospector --profile prospector.yml $$CHANGED_FILES; \
